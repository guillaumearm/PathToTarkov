...
const isValidMap = (mapName: string) => {
  return (
    mapName !== "base" &&
    mapName !== "hideout" &&
    mapName !== "privatearea" &&
    mapName !== "private area" &&
    mapName !== "develop"
  );
};
...
class StashController {
  ...
  _resetSize() {
    ...
    STASH_IDS.forEach((stashId) => {
      this.items[stashId]._props.Grids[0]._props.cellsV =
        this.stashSizes[stashId];
    });
...
class TraderController {
      ...
      if (trader) {
        ...
        if (tradersConfig[traderId].override_description) {
          ...
          Object.keys(this.locales.global).forEach((locale) => {
            ...
            if (locationDescription && locationDescription[locale]) {
              const traderLocales =
                this.locales.global[locale].trading[traderId];
        ...
        if (tradersConfig[traderId].repair_always_enabled) {
          ...
          if (typeof repairTraderConfig.repair_price_coef == "number") {
            trader.base.loyaltyLevels.forEach((payloadLevel) => {
              payloadLevel.repair_price_coef =
                repairTraderConfig.repair_price_coef;
            });
          }
        }
  ...
  updateTraders(offraidPosition, sessionId) {
    ...
    const tradersInfo =
      SaveServer.profiles[sessionId].characters.pmc.TradersInfo;
...
class OffraidRegenController {
  constructor(getConfig) {
    this.regen_db =
      DatabaseServer.tables.globals.config.Health.Effects.Regeneration;
  ...
  init() {
    ...
    Object.keys(this.regen_db.BodyHealth).forEach((bodyPart) => {
      this.bodyhealth_values[bodyPart] = {
        Value: this.regen_db.BodyHealth[bodyPart].Value,
      };
    });
  }
...
class PathToTarkovController {
  ...
  _addSpawnPoint(mapName, spawnPoint) {
    DatabaseServer.tables.locations[mapName].base.SpawnPointParams.push(
      spawnPoint
    );
  }
      ...
      (mapName) => {
        ...
        if (spawnpoints) {
          spawnpoints.forEach((spawnId) => {
            const spawnData =
              this.spawnConfig[mapName] && this.spawnConfig[mapName][spawnId];
            if (spawnData) {
              const spawnPoint = createSpawnPoint(
                getPosition(spawnData),
                spawnData.Rotation,
  ...
  initExfiltrations() {
    ...
    if (!this.config.bypass_exfils_override) {
      ...
      Object.keys(this.config.exfiltrations).forEach((mapName) => {
        ...
        database.locations[mapName].base.exits = extractPoints.map(
...
class PathToTarkov {
  constructor() {
    ...
    const pathToTarkovController = new PathToTarkovController(
      config,
      spawnConfig
    );
    ...
    ModLoader.onLoad[mod.name] = function () {
      ...
      InraidController.saveProgress = (offraidData, sessionId) => {
        ...
        const currentLocationName =
          SaveServer.profiles[sessionId].inraid.location.toLowerCase();
      ...
      MatchController.endOfflineRaid = (info, sessionId) => {
        endRaidCb = (currentLocationName, isPlayerScav) => {
          if (
            isPlayerScav &&
            !pathToTarkovController.config.player_scav_move_offraid_position
          ) {
            return;
          }
          ...
          if (
            pathToTarkovController.config
              .reset_offraid_position_on_player_die &&
            playerDied
          ) {
            pathToTarkovController.updateOffraidPosition(
              sessionId,
              pathToTarkovController.config.initial_offraid_position
            );
            return;
          }

          const extractsConf =
            pathToTarkovController.config.exfiltrations[currentLocationName];
